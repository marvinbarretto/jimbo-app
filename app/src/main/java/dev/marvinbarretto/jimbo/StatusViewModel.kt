...
import dev.marvinbarretto.jimbo.data.CollectorEventSummary
import dev.marvinbarretto.jimbo.data.StepsDatabase
import dev.marvinbarretto.jimbo.telemetry.currentBatterySnapshot
import dev.marvinbarretto.jimbo.telemetry.currentNetworkSnapshot
...
class StatusViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val prefs = application.getSharedPreferences("jimbo_sync", Context.MODE_PRIVATE)
    ...
    fun refreshStatus() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            _state.value = StatusUiState(
                ...
                syncRequestInFlight = workInfos.any { it.state == WorkInfo.State.RUNNING }
            )
        }
    }
